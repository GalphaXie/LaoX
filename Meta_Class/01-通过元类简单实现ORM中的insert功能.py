    ...
    def __new__(cls, class_name, bases, class_attrs):
        ...
        for k, v in class_attrs.items():
            if isinstance(v, tuple):  # 判断v是否是元组而不是k，下面调用的时候传递的是元组；Djoingo框架不是元组而是类，
                ...
                mappings[k] = v
        print("class_attrs: %s" % class_attrs)
...
{'__mappings__':
     {'password': ('password', 'varchar(30)'), 'name': ('username', 'varchar(30)'), 'email': ('email', 'varchar(30)'), 'uid': ('uid', 'int unsigned')},
 '__init__': '<function User.__init__ at 0x7f14185741e0>',
 '__qualname__': 'User',
 '__module__': '__main__',
 'save': '<function User.save at 0x7f1418574268>',
 '__table__': 'User'}
